...
def test_reg_account_exists_with_email(web_browser):
    """ Проверка невозможности регистрации с адресом почты уже зарегистрированного пользователя """
    ...
    page.phone_email_input.send_keys(EMAIL_VALID)
    ...
    assert page.account_exists_text.find()  # проверка появившегося окна с текстом, что учетная запись уже существует
